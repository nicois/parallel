...
func main() {
	...
	nctx, ncancel := signal.NotifyContext(ctx, os.Interrupt, syscall.SIGTERM, syscall.SIGHUP)
	defer ncancel()
	go func() {
		<-nctx.Done()
		cancelCause(parallel.UserCancelled)
	}()
	...
	if opts.DeferReruns {
		...
		go func() {
			...
			for uc := range presortedCommands {
				postSortedCommands <- uc.command
			}
		}()
	}
	...
	err = parallel.Run(ctx, stats, opts, postSortedCommands)
