...
- define timeouts
- perform graceful job cancellation (waiting for running jobs to finish before exiting)
...
```bash
$ seq 1 254 | parallel --hide-failures --graceful-exit --concurrency 100 --debounce 10s --timeout 10s -- nc -vz 192.168.4.{{.value}} 443
...
```bash
$ seq 1 10 \
    | parallel --concurrency 2 -- bash -c 'echo {{.value}} ; sleep {{.value}}'
Dec  4 12:53:52.426 INF Success command="{command:[bash -c echo 1 ; sleep 1] input:}" "combined output"="1\n"
Dec  4 12:53:53.426 INF Success command="{command:[bash -c echo 2 ; sleep 2] input:}" "combined output"="2\n"
^CDec  4 12:53:53.731 WRN job was aborted due to context cancellation command="{command:[bash -c echo 4 ; sleep 4] input:}"
Dec  4 12:53:53.731 WRN Failure command="{command:[bash -c echo 4 ; sleep 4] input:}" "combined output"="4\n" error="signal: killed"
Dec  4 12:53:53.731 WRN job was aborted due to context cancellation command="{command:[bash -c echo 3 ; sleep 3] input:}"
Dec  4 12:53:53.731 WRN Failure command="{command:[bash -c echo 3 ; sleep 3] input:}" "combined output"="3\n" error="signal: killed"
Dec  4 12:53:53.731 INF Queued: 6; Skipped: 0; In progress: 0; Succeeded: 2; Failed: 2; Total: 10; Estimated time remaining: 3s
Dec  4 12:53:53.731 ERR context canceled
```
...
```bash
$ seq 1 10 \
    | parallel --concurrency 2 --graceful-exit -- bash -c 'echo {{.value}} ; sleep {{.value}}'
Dec  4 12:54:21.407 INF Success command="{command:[bash -c echo 1 ; sleep 1] input:}" "combined output"="1\n"
Dec  4 12:54:22.408 INF Success command="{command:[bash -c echo 2 ; sleep 2] input:}" "combined output"="2\n"
^CDec  4 12:54:22.657 INF received cancellation signal. Waiting for current jobs to finish before exiting
Dec  4 12:54:22.657 INF Queued: 6; Skipped: 0; In progress: 2; Succeeded: 2; Failed: 0; Total: 10; Elapsed time: 2s
Dec  4 12:54:24.413 INF Success command="{command:[bash -c echo 3 ; sleep 3] input:}" "combined output"="3\n"
Dec  4 12:54:26.413 INF Success command="{command:[bash -c echo 4 ; sleep 4] input:}" "combined output"="4\n"
Dec  4 12:54:26.413 INF Queued: 6; Skipped: 0; In progress: 0; Succeeded: 4; Failed: 0; Total: 10; Estimated time remaining: 9s
Dec  4 12:54:26.413 ERR context canceled
```

If you want to stop processing if a job fails, use `--abort-on-error`:
(this can be combined with `--graceful-exit`)
